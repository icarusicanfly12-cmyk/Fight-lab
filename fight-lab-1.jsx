...
export default function FightLab() {
  ...
  return (
    <div style={s.app}>
      ...
      {tab === "nutrition" && (
        <div>
          <div style={{ background: "#0f0f0f", margin: "12px 14px", borderRadius: "14px", padding: "18px", border: "1px solid #181818" }}>
            <div style={{ fontFamily: "'Oswald', sans-serif", fontSize: "10px", letterSpacing: "3px", color: "#333", marginBottom: "14px" }}>DAILY MACROS — LEAN BULK</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "6px", marginBottom: "14px" }}>
              {[
                { l: "CAL", v: "2700", c: "#ef4444" },
                { l: "PRO", v: "140g", c: "#f97316" },
                { l: "CARB", v: "365g", c: "#eab308" },
                { l: "FAT", v: "75g", c: "#22c55e" },
              ].map(m => (
                <div key={m.l} style={{ textAlign: "center", background: "#080808", borderRadius: "8px", padding: "10px 4px" }}>
                  <div style={{ fontFamily: "'Oswald', sans-serif", fontSize: "16px", fontWeight: 700, color: m.c }}>{m.v}</div>
                  <div style={{ fontSize: "9px", color: "#333", letterSpacing: "1px", marginTop: "2px" }}>{m.l}</div>
                </div>
              ))}
            </div>
            <div style={{ fontSize: "11px", color: "#2e2e2e", padding: "10px", background: "#080808", borderRadius: "8px" }}>
              170cm · 60kg · 5 days training/wk · Target 65kg @ 10-12% BF
            </div>
          </div>

          <div style={s.sec}>MEAL PLAN</div>
          {meals.map((meal, i) => (
            <div key={meal.name} style={s.card}>
              <div onClick={() => setExpandedMeal(expandedMeal === i ? null : i)} style={{ padding: "14px 16px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <div style={{ fontFamily: "'Oswald', sans-serif", fontSize: "16px", fontWeight: 600 }}>{meal.name}</div>
                  <div style={{ fontSize: "11px", color: "#444", marginTop: "2px" }}>{meal.time} · {meal.cal} kcal</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: "11px", color: "#f97316" }}>P {meal.p}g</div>
                  <div style={{ fontSize: "11px", color: "#eab308" }}>C {meal.c}g</div>
                  <div style={{ fontSize: "11px", color: "#22c55e" }}>F {meal.f}g</div>
                </div>
              </div>
              {expandedMeal === i && (
                <div style={{ borderTop: "1px solid #111", padding: "12px 16px" }}>
                  {meal.foods.map((f, fi) => (
                    <div key={fi} style={{ padding: "7px 0", fontSize: "13px", color: "#777", borderBottom: fi < meal.foods.length - 1 ? "1px solid #111" : "none", display: "flex", gap: "8px" }}>
          ...
          <div style={s.sec}>KEY RULES</div>
          <div style={s.card}>
            <div style={{ padding: "14px 16px" }}>
              {[
                "Protein within 30 min post-training — non-negotiable",
                "Peanuts, eggs, dal — cheapest protein in Pune",
                "Don't cut carbs — you need them for MMA energy",
                "3.5L water on training days minimum",
                "Budget tight? Protect protein + carbs over fat",
              ].map((tip, i) => (
                <div key={i} style={{ padding: "9px 0", fontSize: "13px", color: "#666", borderBottom: i < 4 ? "1px solid #111" : "none", display: "flex", gap: "10px" }}>
                  <span style={{ color: "#ef4444", flexShrink: 0, fontFamily: "'Oswald', sans-serif" }}>→</span>
                  <span>{tip}</span>
                </div>
              ))}
            </div>
      ...
      <nav style={s.nav}>
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id)} style={s.nb(tab === t.id)}>
...
function ProgressTab({ currentWeek, setWeek, percent, doneCount, notes, saveNote, phases, s }) {
  const [noteInput, setNoteInput] = useState(notes[currentWeek] || "");
  ...
  return (
    <div>
      <div style={{ background: "#0f0f0f", margin: "12px 14px", borderRadius: "14px", padding: "22px", border: "1px solid #181818" }}>
        <div style={{ fontFamily: "'Oswald', sans-serif", fontSize: "10px", letterSpacing: "3px", color: "#333", marginBottom: "10px" }}>6-WEEK EXPERIMENT PROGRESS</div>
        <div style={{ fontFamily: "'Oswald', sans-serif", fontSize: "56px", fontWeight: 700, color: "#ef4444", lineHeight: 1 }}>{percent}%</div>
        <div style={{ fontSize: "12px", color: "#444", marginBottom: "16px" }}>{doneCount} exercises completed across all weeks</div>
        <div style={{ background: "#1a1a1a", borderRadius: "6px", height: "8px", overflow: "hidden" }}>
          <div style={{ background: "linear-gradient(90deg, #ef4444, #f97316)", height: "100%", width: `${percent}%`, transition: "width 0.5s", borderRadius: "6px" }} />
        </div>
      </div>

      <div style={s.sec}>WEEK TRACKER</div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "8px", padding: "0 14px" }}>
        {[1,2,3,4,5,6].map(w => {
          const phase = w <= 2 ? phases[0] : w <= 4 ? phases[1] : phases[2];
          const isActive = w === currentWeek;
          const isPast = w < currentWeek;
          return (
            <div key={w} onClick={() => setWeek(w)} style={{ background: "#0f0f0f", border: `2px solid ${isActive ? phase.color : isPast ? "#1e1e1e" : "#141414"}`, borderRadius: "12px", padding: "14px", cursor: "pointer", textAlign: "center", transition: "border-color 0.2s" }}>
              <div style={{ fontFamily: "'Oswald', sans-serif", fontSize: "22px", fontWeight: 700, color: isActive ? phase.color : isPast ? "#2a2a2a" : "#444" }}>W{w}</div>
              <div style={{ fontSize: "9px", color: isActive ? phase.color : "#2a2a2a", letterSpacing: "1px", fontFamily: "'Oswald', sans-serif", marginTop: "2px" }}>{phase.label}</div>
              {isActive && <div style={{ marginTop: "8px", width: "6px", height: "6px", borderRadius: "50%", background: "#ef4444", margin: "8px auto 0" }} />}
              {isPast && <div style={{ marginTop: "4px", fontSize: "14px" }}>✓</div>}
            </div>
          );
        })}
      </div>
      ...
      <div style={s.card}>
        <div style={{ padding: "14px" }}>
          <textarea
            ...
            placeholder={`Week ${currentWeek} — How's the training? Gas tank? Technique improving? What's working, what's not...`}
            style={{ width: "100%", background: "#080808", border: "1px solid #1e1e1e", borderRadius: "10px", color: "#888", fontSize: "13px", padding: "14px", minHeight: "110px", resize: "none", fontFamily: "'Barlow', sans-serif", lineHeight: "1.5" }}
          />
          <div style={{ fontSize: "10px", color: "#222", marginTop: "6px", letterSpacing: "1px" }}>AUTO-SAVES ON TAP AWAY</div>
        </div>
      </div>
      ...
      <div style={s.card}>
        <div style={{ padding: "14px 16px" }}>
          {[
            "Gas tank — how many rounds before you gas out?",
            "Technique — are combos feeling smoother week on week?",
            "Conditioning — can you push harder than week 1?",
            "Body weight — staying on lean bulk trajectory?",
            "App value — does structure actually help or feel generic?",
          ].map((item, i) => (
            <div key={i} style={{ padding: "9px 0", fontSize: "13px", color: "#555", borderBottom: i < 4 ? "1px solid #111" : "none", display: "flex", gap: "10px" }}>
              <span style={{ color: "#ef4444", flexShrink: 0, fontFamily: "'Oswald', sans-serif", fontWeight: 700 }}>{i + 1}</span>
              <span>{item}</span>
            </div>
          ))}
        </div>
